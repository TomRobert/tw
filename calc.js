...
TW_Calc = {
	...
	gameMIN: "1.36",
	website: "https://beta.tw-calc.net",
...
TW_Calc.loadLangPack = function(){
	...
	$.ajax({url:TW_Calc.website+"/service/get-script-language-pack?lang="+TW_Calc.getAvailableLang()+"&"+(new Date().toDateString())+"&"+TW_Calc.version, dataType:"jsonp", timeout: 4000, success: function(){
...
TW_Calc.functions.settings = function(){
	...
	new west.gui.Dialog(TW_Calc.getTranslation(93), msg).addButton('ok', ok).addButton(TW_Calc.getTranslation(92), zrus).show();
};
...
TW_Calc.TWCalcMyinfo.launch = function(){
    ...
    var OTHER_BONUSES = {defense: 0, offense: 0, damage: 0, resistance: 0};
    var weapon = {damage_min: 50, damage_max:110};
    var charLvl = Character.level;
    var sets = {};
	
	for (var k in Wear.wear){
        var obj = Wear.wear[k].obj;
        var bon = obj.bonus;
        OTHER_BONUSES.defense += bon.fortbattle.defense + bon.fortbattlesector.defense;
        OTHER_BONUSES.offense += bon.fortbattle.offense + bon.fortbattlesector.offense;
        OTHER_BONUSES.damage += bon.fortbattlesector.damage;
        OTHER_BONUSES.resistance += bon.fortbattle.resistance;
        if (k == 'left_arm')
            weapon = obj.damage;
        for (var l=0;l<bon.item.length;l++){
            var ID = bon.item[l];
            var NAM = ID.name || ID.bonus && (ID.bonus.name || ID.bonus.type) || ID.type;
            if (OTHER_BONUSES.hasOwnProperty(NAM) && (NAM == 'damage' && k != 'right_arm')){
                var VAL = (ID.value !== undefined ? ID.value : ID.bonus.value);
                var NUM = ID.key && Math.ceil(VAL * charLvl) || Math.round(VAL*100)/100;
                OTHER_BONUSES[NAM] += NUM + ItemManager.calculateItemLevelBonus(obj.item_level, NUM);
            }
        }
        if (obj.set){
            if (!sets[obj.set])
    ...
    for (var m in sets){
        var arr = west.storage.ItemSetManager._setList[m].getMergedStages(sets[m]);
        for (var n = 0; n < arr.length; n++){
            var ID = arr[n];
            var NAM = ID.name || ID.bonus && (ID.bonus.name || ID.bonus.type) || ID.type;
            if (OTHER_BONUSES.hasOwnProperty(NAM)){
                var VAL = (ID.value || ID.bonus.value);
                OTHER_BONUSES[NAM] += ID.key && Math.ceil(VAL * charLvl) || Math.round(VAL*100)/100;
            }
        }
	}
    ...
    var input = {
        ...
        map_position: 0,
        char_tower: false,
        damage: [weapon.damage_min, weapon.damage_max],
        bonus: {
            attack: OTHER_BONUSES.offense,
            defense: OTHER_BONUSES.defense,
            resistance: OTHER_BONUSES.resistance,
            damage: OTHER_BONUSES.damage
        }
    };
	...
	try{
		...
		if(TW_Calc.bool(localStorage.getItem("TWCalc_Settings")) !== false){
			...
			if(data.hasOwnProperty(name)){
				...
				if(type != 'number') return true; else return 1;
...
TW_Calc.val = function(d){
	...
	if($('#tab_twcalc7').find(selector_button).length !== 0){
		...
		if($(selector_button).hasClass("tw2gui_checkbox_checked")) return true; else return false;
...
TW_Calc.craft.ItemCraft = function(id, s, reciepeId, craftable){
	...
	if(typeof(itemObj.item)=="object") item.item = itemObj.item.item_id; else item.item = itemObj.item;
...
TW_Calc.craft.quests = function(){
	try {
		...
		Quest.backup_getMinimapLink = Quest.getMinimapLink;
		Quest.getMinimapLink = function (jsRequirement) {
			...
			if (jsRequirement && jsRequirement.id && jsRequirement.type == 'inventory_changed'){
				...
				if (isDefined(tmpObj) && tmpObj.spec_type=='crafting'){
					mmLink='<span class="quest_craftlink" style="cursor:pointer;" title=\''+TW_Calc.getTranslation(192)+'\' onclick="TW_Calc.craft.show('+tmpObj.item_id+')"><img src="https://tomrobert.safe-ws.de/craft.png"/></span>&nbsp;';
					return mmLink;
				}
			}
			...
			return Quest.backup_getMinimapLink(jsRequirement);
		};
...
TW_Calc.nearestJob.find = function(e, dataType){
		...
		if(TW_Calc.nearestJob.map != null) var q = TW_Calc.nearestJob.map; else TW_Calc.nearestJob.getMap();
...
TW_Calc.nearestJob.build = function(){
	...
	if(TW_Calc.bool(localStorage.getItem("TWCalc_jobList"))!== false){
		...
		if(localStorage.getItem("TWCalc_jobList") != '') TW_Calc.nearestJob.list = localStorage.getItem("TWCalc_jobList").split(","); else TW_Calc.nearestJob.list = [];
...
TW_Calc.nearestJob.fade = function(s){
	...
	if(s.css("opacity") == 1) s.css("opacity", 0.5); else s.css("opacity", 1);
...
TW_Calc.BottomBarMover = function(){
	...
	TW_Calc.nearestJob.int = setInterval(function(){
		...
		if((TW_Calc.Settings.get("topBar", "number") == 1) || (TW_Calc.Settings.get("duelBar", "number") == 2)){
			...
			if($("#ui_windowdock").css("display") == "none" || $('.windowbar_frames').html() == '')	n = n + 15; else n = n + 47;
...
try{
	...
	TW_Calc.TombolaExporter.Tombola = function(){
		...
		west.wof.WheelofFortune.prototype.process = function(action, data, callback, context, window){
            ...
            if(TW_Calc.ShowLogs) console.log(data);
        ...
        west.wof.WofDotdCardgameWindow.__proto__.requestData = function (action, data, callback) {
            ...
            Ajax.remoteCall('wheeloffortune', 'gamble', data, function (response) {
                if(TW_Calc.ShowLogs) console.log(JSON.stringify(response));
                if (response.error) {
	...
	TW_Calc.TombolaExporter.createData = function(a, z){
		try{
            ...
            if(TW_Calc.ShowLogs) console.log(a);
            ...
            else if(b == 7 || b == 8 || b == 14 || b == 15){
                
                if (isDefined(a.construction_id))
                TW_Calc.TombolaExporter.level = a.construction_id;
            
                if(TW_Calc.bool(a) === true && !a.failed){
                    var prize = a.itemId || a.outcome && a.outcome.itemId;
                    var c = a.itemEnhance || a.outcome && a.outcome.itemEnhance;
                    
                    var category = 0;
                    switch(c){
                        case 25: category = 1; break;
                        case 150: category = 2; break;
                        case 800: category = 3; break;
                    }
                    
                    var level = TW_Calc.TombolaExporter.level;
	...
	TW_Calc.TombolaExporter.exportData = function(prize, id, category, level){
        ...
        if(TW_Calc.ShowLogs) console.log(level);
        if(TW_Calc.bool(level) !== false){
	...
	TW_Calc.TombolaExporter.saveData = function(prize,tombolaId,category){
		try{
			...
			if(tombolaId == 1 || tombolaId == 11 || tombolaId == 12 || tombolaId == 15){
		...
		try{
            ...
            for(var i=0;i < obj.length;i++){
                ...
                if(Object.keys(oi).length > 0){
                    var bg = '';
                    switch(i){
                        case 0: bg = 'rgba(128, 128, 128, 0.4)'; break;
                        case 1: bg = 'rgba(0, 128, 0, 0.4)'; break;
                        case 2: bg = 'rgba(0, 0, 255, 0.4)'; break;
                        case 3: bg = 'rgba(255, 215, 0, 0.4)'; break;
                        case 4: bg = 'rgba(255, 0, 0, 0.4)'; break;
                    }
		...
		try {
			for (var i = 0; i < resObj.msg.effects.length; i += 1) {
				...
				if (res.type == 'lottery' || res.type == 'content') {
					...
					if(TW_Calc.ShowLogs) console.log(cont);
	...
	TW_Calc.Chests.open = function(){
		try {
			...
			if (!localStorage.getItem('TWCalc_chests_1_08')){
				
				localStorage.setItem('TWCalc_chests_1_08', 'done');
				
				var twdbChests = localStorage.getItem('twdb_'+Character.playerId+'_statistic');
				
				if (twdbChests){
					
					var chests = $.parseJSON(decodeURIComponent(twdbChests)).chest
				
					for (var j in chests)
						if (chests[j].count && chests[j].items) {
							if(TW_Calc.ShowLogs) console.log(j.toString());
							$.get(TW_Calc.website+'/service/chest-export',{chest:j,count:chests[j].count,content:chests[j].items,version:Game.version},function(data){},"jsonp");
						}
...
TW_Calc.isBirthday = function(){
	...
	if(TW_Calc.birthday_enabled === true){
		...
		if(month == TW_Calc.birhday.month && day == TW_Calc.birhday.day) return true; else return false;
...
TW_Calc.DuelBar.loadPlayerData = function(){
	...
	$.getJSON("/game.php?window=duel&action=search_op&h="+Player.h, complete = function(xhr){
		...
		if(TW_Calc.DuelBar.loadedData.length !== 0){
			...
			if(TW_Calc.Settings.get("topBar", "number") != 2) if($("#WESTCALC_TOP_BAR>#Westcalc_DuelBar").html() == '') $("#WESTCALC_TOP_BAR").remove();
...
TW_Calc.birthaday = function(){
	...
	var y3 = y1 - y2;
	var y;
	if(y3 == 1) y = '1st'; else if(y3 == 2) y = '2nd'; else if(y3 == 3) y = '3rd'; else y = y3+'th';
...
var setVal = setInterval(function () {
    if (Character && Character.playerId) {
        ...
        try{
			...
			TW_Calc.Settings.list = [["topBar", TW_Calc.getTranslation(185), true], ["duelBar", TW_Calc.getTranslation(191), true], ["wardrobe", TW_Calc.getTranslation(175)], ["MenuCraftButton", TW_Calc.getTranslation(153)], ["TransferFeeCalc", TW_Calc.getTranslation(108)],["XP_HP_Energy_Calc", TW_Calc.getTranslation(109)], ["westcalc", TW_Calc.getTranslation(184)]];
